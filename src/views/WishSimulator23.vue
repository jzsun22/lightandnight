  ...
  <van-tabs v-model:active="active" sticky offset-top="46">
    ...
    <van-tab :title="data[volume].name">
      <van-cell-group :title="$t('common.gacha')" inset>
        <van-cell
          v-if="remainGifts"
          :title="$t('wishsim23.gacha')"
          is-link
          @click="gacha3Times"
        />
        <van-cell
          v-else
          :title="$t('wishsim23.complete', [gachaTime, data[volume].gift])"
        />
      </van-cell-group>

      <van-cell-group :title="$t('common.currentStatus')" inset>
        <van-cell
          :title="$t('wishsim23.possibility', [data[volume].gift])"
          :value="giftPossibilityString"
          is-link
          @click="possibilityShow = true"
        />
        <van-cell>
          {{ $t('wishsim23.gachaTime') }}
          <van-progress
          ...
          />
          <small v-text="$t('wishsim23.gachaTimeDesc', data[volume].must)" />
        </van-cell>
      </van-cell-group>
    ...
    <van-tab :title="$t('common.settings')">
      <van-radio-group v-model="maxFrame">
        <van-cell-group :title="$t('wishsim23.settings.maxFrame')" inset>
          <van-cell
            v-for="v in maxFrames"
            :key="v"
            :title="$t(`wishsim23.settings.maxFrames[${v}]`)"
            clickable
            @click="setMaxFrame(v)"
          >
            <template #right-icon>
      ...
      <van-cell-group
        v-if="volume === 0"
        :title="$t('wishsim23.settings.weight')"
        inset
      >
        <van-cell
          :title="$t('common.info')"
      ...
      <div class="container">
        <van-row class="content center-row">
          <van-col v-for="i in [0, 1, 2]" :key="i" span="8">
            <strong v-t="`wishsim23.possibilityTitle[${i}]`" />
          </van-col>
        </van-row>
        <van-divider dashed />
      ...
      <div class="container">
        <van-row v-for="i in [0, 1]" :key="i" class="content">
          <van-col span="24" class="desc">
            {{ $t(`wishsim23.possibilityDesc[${i}]`, [data[volume].gift]) }}
          </van-col>
        </van-row>
      </div>
    </van-dialog>
      ...
      let animate = () => {
        ...
        if (this.frame <= this.maxFrame) requestAnimationFrame(animate)
          ...
          if (isGift)
            showNotify({
              type: 'success',
              message: this.$t('wishsim23.notify.success', [
          ...
          if (!isGift) {
            for (let i of [0, 1, 2]) {
              if (
                ...
                showNotify({
                  type: 'success',
                  message: this.$t('wishsim23.notify.protect', [
